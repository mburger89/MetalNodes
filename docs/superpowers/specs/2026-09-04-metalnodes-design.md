...
- **§23.4 — `sys` supplies `resolution` as `float2(1.0, 1.0)` and `mouse` as `float2(0.0, 0.0)`.** Every group function's signature begins `(float2 uv, float time, float2 size, float2 mouse, …)` and the UV node's `aspect` variant reads `{sys.resolution}`, so both keys must resolve. No node can observe them: Resolution and Mouse are refused under this target.
