...
fragment float4 shaderMain(VertexOut in [[stage_in]],
    ...
    v0 = u.p0;
    float v1;
    v1 = mn_valueNoise(params.geometry().uv0() * u.p5);
    float4 baseColor = v0;
    float4 emissive = u.p2;
    float opacity = u.p7;
    float3 tangentNormal = u.p1;
    float roughness = clamp(v1, 0.03, 1.0);
    float metallic = saturate(u.p6);
    float occlusion = saturate(u.p8);
    float specular = saturate(u.p9);
