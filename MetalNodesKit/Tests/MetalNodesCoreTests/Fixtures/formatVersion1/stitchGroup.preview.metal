...
half4 stitchGroup(float2 position, half4 currentColor, float2 size, float time, float2 mouse, half4 p0, float p1, float p2) {
...
fragment float4 shaderMain(VertexOut in [[stage_in]],
    ...
    float2 position = float2(in.uv.x, 1.0 - in.uv.y) * u.resolution;
    half4 c = stitchGroup(position, half4(0.0), u.resolution, u.time, u.mouse, half4(u.p0), u.p1, u.p2);
    return float4(c);
}
