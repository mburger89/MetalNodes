...
- **Custom MSL is confined to `groupFunction`'s vocabulary, permanently, and that is a real limitation nothing else in §24 records.** A `.msl` body can read `uv`, `time`, `size` and `mouse` — its four system parameters — plus its own declared inputs as `in_<name>`. Everything else must arrive through an input socket. Three cases, and only the third is a dead end:
  ...
  - **Has no workaround.** `params.surface()` appears in no `materialSys` spelling and is exposed by no builtin node, so a Custom MSL body cannot see it: reading back a surface property the graph has already set is not expressible. *(Prediction, not shipped behaviour: `params.uniforms().custom_parameter()` would be in this same position. §24.6 is **unimplemented as of M8's legality work** — `custom_parameter` appears nowhere in the tree — so when live parameters land, a live value will be able to drive a node graph but not a hand-written body, unless it is also exposed as an ordinary definition input, in which case it arrives baked as a literal in the export rather than live. Whoever implements §24.6 should confirm or refute this rather than inherit it.)*
